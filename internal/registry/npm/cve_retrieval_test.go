...
// newCveRetrievalAdapter builds the npm adapter with a retrieval chain of
...
// expiry.
func newCveRetrievalAdapter(t *testing.T, osvURL, mode string, client RegistryClient, store Store, now func() time.Time) *npmAdapter {
	t.Helper()
	dir := t.TempDir()
	...
	validation, err := reg.BuildValidation([]config.Middleware{
		{Type: "min-publication-age", Params: yamlNode("min_days: 0")}, // age gate off; retrieval cve-check gates
	})
	if err != nil {
		t.Fatal(err)
	}
...
// TestCveRetrievalDeniesStoredPackageAfterAdvisory proves the retrieval stage
// re-checks OSV on a serve from storage: a package stored while OSV was clean is
...
func TestCveRetrievalDeniesStoredPackageAfterAdvisory(t *testing.T) {
	osv, ctrl := newOSV(t)
	store := newMemStore()
	...
	a := newCveRetrievalAdapter(t, osv.URL, "deny", client, store, func() time.Time { return now })
	...
	if code != 200 || string(body) != "TARBALL" {
		t.Fatalf("request 1: code=%d body=%q want 200/TARBALL", code, body)
	}
	if len(store.recs) != 1 {
		t.Fatalf("request 1 should store the validated package, got %d records", len(store.recs))
	}
	if ctrl.hits.Load() != 1 {
...
// TestCveRetrievalWarnServesStoredPackage proves mode warn serves the stored
// package while still re-querying OSV and annotating the request.
func TestCveRetrievalWarnServesStoredPackage(t *testing.T) {
	osv, ctrl := newOSV(t)
	store := newMemStore()
	pack, raw := buildPack(time.Now().AddDate(0, 0, -30), []byte("TARBALL"))
	client := &rawClient{pack: pack, raw: raw, tarball: []byte("TARBALL")}
	...
	now := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	a := newCveRetrievalAdapter(t, osv.URL, "warn", client, store, func() time.Time { return now })
	srv := newTestServer(t, a)
	...
	code, body := fetchViaProxy(t, srv.URL+"/npm", "testpkg", "1.0.0")
	if code != 200 || string(body) != "TARBALL" {
		t.Fatalf("request 2 (warn): code=%d body=%q want 200/TARBALL", code, body)
	}
	if delta := ctrl.hits.Load() - before; delta != 1 {
